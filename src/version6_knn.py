...
logging.basicConfig(
    ...
    filename="anime_knn_recommender_full.log"
)
logger = logging.getLogger(__name__)

class AnimeKNNRecommender:
    """KNN-based anime recommendation system using full data."""
    ...
    def create_optimization_dataset(self, X_train, y_train):
        ...
        if len(X_train) > self.optimization_size:
            ...
            X_opt, _, y_opt, _ = train_test_split(
                ...
                random_state=42,
                stratify=y_train.round().astype(int)  # Stratify by rating class
            )
        else:
            X_opt = X_train
            y_opt = y_train
    ...
    def train_model(self, X_train, y_train):
        """Train the KNN model."""
        print(f"Training KNN model with {self.n_neighbors} neighbors on {len(X_train)} samples...")
    ...
    def evaluate(self, X_test, y_test):
        ...
        # Round and clip predictions to match rating scale (1-10 integers)
        ...
        # Calculate accuracy (exact match after rounding)
        accuracy = np.mean(y_pred_rounded == np.round(y_test).astype(int))
        
        # Calculate "within 1 point" accuracy
        within_one = np.mean(abs(y_pred_rounded - np.round(y_test).astype(int)) <= 1)
    ...
    def generate_recommendations(self, user_id, top_n=10):
        ...
        try:
            ...
            # Remove columns not in feature set
            cols_to_drop = ['user_id', 'anime_id', 'name']
            if 'rating' in test_features.columns:
                cols_to_drop.append('rating')
            ...
            # Round ratings
            recommendations['predicted_rating'] = np.round(
                np.clip(recommendations['predicted_rating'], 1, 10)
            ).astype(int)
    ...
    def save_model(self, filepath='anime_knn_model', directory='models', format='joblib'):
...
def main():
    """Main function to run the KNN recommendation system using full data."""
    print("Starting KNN anime recommender with DTRF-style preprocessing...")
    
    # Create recommender
    recommender = AnimeKNNRecommender(
        n_neighbors=10,  # Initial value, will be optimized
        n_components=20,
    ...
    try:
        ...
        # Save the model
        recommender.save_model('anime_knn_model_dtrf_style', directory='models')
        ...
        if len(train_data['user_id'].unique()) > 0:
            ...
            if recommendations is not None:
                ...
                print(recommendations[['name', 'predicted_rating']].to_string(index=False))
        
        print("\nKNN analysis with DTRF-style preprocessing complete!")
        
    except Exception as e:
        logger.error(f"Error in KNN model: {e}")
